...
const MAX_CONCURRENT_REQUESTS = 5; // Number of simultaneous requests
const RETRY_COUNT = 3; // Retry limit for failed network requests
...
async function fetchHTMLWithRetry(url, retries = RETRY_COUNT) {
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (attempt === retries) {
        throw new Error(`Failed to fetch URL after ${retries} attempts: ${url}`);
      }
  ...
  const promises = alerts.map(async (alert) => {
    try {
      ...
      if (currentPrice <= alert.targetPrice) {
        const emailContent = generateEmailTemplate(alert.title, currentPrice, alert.url);
        ...
        console.log(`Price drop email sent to ${alert.userEmail} for ${alert.url}`);
